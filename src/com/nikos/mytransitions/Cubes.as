...
public class Cubes extends Transition
{
	...
	*/
	for(var i=0;i<cubes.length;i++)	
	{
		...
		zz=cubes[i].z;
		z0 = zz * param3.z_multiplier;
		if (param3.direction == "up" || param3.direction == "down")
		{
			pos = cubes[i].x;
			pos0 = cubes[i].x + cubes[i].x * 0.1;
			TweenMax.to(cubes[i], d, {delay:ordobj.delays[i]*del, useFrames:useFrames, immediateRender:startNow, rotationX:cubes[i].extra.targetRotation, x:pos, z:zz, bezierThrough:[{x:pos0, z:z0}], ease:param3.easing, onComplete:doall});
		}
		else
		{
			pos0 = cubes[i].y + cubes[i].y * 0.1;
			pos = cubes[i].y;
			TweenMax.to(cubes[i], d, {delay:ordobj.delays[i]*del, useFrames:useFrames, immediateRender:startNow, rotationY:cubes[i].extra.targetRotation, y:pos, z:zz, bezierThrough:[{y:pos0, z:z0}], ease:param3.easing, onComplete:doall});
		}
	}
